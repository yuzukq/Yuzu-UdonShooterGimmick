...
public class GridShotManager : UdonSharpBehaviour
{
    ...
    //HUD設定をPlayerManagerに移行
    /*[Header("HUD設定")]
    [SerializeField] Transform GameHUDPosition;
    ...
    */
    ...
    private void EndGame()
    {
        ...
        //----------------HUD表示-------------------------------
        playerManager.ResultScoreText.text = $"Score: {erlierScore.ToString()}";
        playerManager.GetPointText.text = $"Point: {shopManager.upGradePoints.ToString()}pt";
        //isHUDActive = true;
        GameSESource.PlayOneShot(HUDSEClip);
        playerManager.ShowGridShotResultHUD();
        //playerManager.ResultHUD.localScale = Vector3.one; // HUDを表示
        ...
        ///    //Vector3 position = Networking.LocalPlayer.GetTrackingData(VRCPlayerApi.TrackingDataType.Head).position;
            //Quaternion rotation = Networking.LocalPlayer.GetTrackingData(VRCPlayerApi.TrackingDataType.Head).rotation;
        ///    //playerManager.GameHUDPosition.SetPositionAndRotation(position, rotation);
        ///}
        
        //SendCustomEventDelayedSeconds("HideResultHUD", 5.0f); // 5秒後にHUDを非表示にする
        //-----------------------------------------------
    }

    /*
    public void HideResultHUD()
    {
        isHUDActive = false;
        playerManager.ResultHUD.localScale = Vector3.zero; // HUDを非表示
    }
    */
